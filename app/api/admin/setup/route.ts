...
export async function GET() {
  try {
    ...
    return NextResponse.json(
      {
        ok: false,
        error: error instanceof Error ? error.message : "Errore setup",
      },
      { status: 500 }
    );
  }
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (Number(rows[0]?.totale ?? 0) > 0) {
      return NextResponse.json(
        { ok: false, error: "Setup già completato" },
        { status: 403 }
      );
    }
    ...
    if (!nome || !email || !password) {
      return NextResponse.json(
        { ok: false, error: "Nome, email e password sono obbligatori" },
        { status: 400 }
      );
    }
