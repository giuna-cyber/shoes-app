...
CREATE TABLE IF NOT EXISTS varianti (
  id BIGSERIAL PRIMARY KEY,
  articolo_id BIGINT NOT NULL REFERENCES articoli(id) ON DELETE CASCADE,
  taglia_id BIGINT REFERENCES taglie(id) ON DELETE SET NULL,
  colore_id BIGINT REFERENCES colori(id) ON DELETE SET NULL,
...
CREATE TABLE IF NOT EXISTS promozioni (
  ...
  descrizione TEXT,
  tipo VARCHAR(30) NOT NULL CHECK (tipo IN ('percentuale', 'prezzo_fisso')),
  valore NUMERIC(10,2) NOT NULL,
  data_inizio TIMESTAMPTZ,
  data_fine TIMESTAMPTZ,
  attiva BOOLEAN NOT NULL DEFAULT TRUE,
  immagine_url TEXT,
  articolo_id BIGINT REFERENCES articoli(id) ON DELETE CASCADE,
  categoria_id BIGINT REFERENCES categorie(id) ON DELETE CASCADE,
  marca_id BIGINT REFERENCES marche(id) ON DELETE CASCADE,
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS utenti_admin (
  ...
  email VARCHAR(190) NOT NULL UNIQUE,
  password_hash TEXT NOT NULL,
  ruolo VARCHAR(30) NOT NULL DEFAULT 'admin' CHECK (ruolo IN ('admin', 'operatore')),
  attivo BOOLEAN NOT NULL DEFAULT TRUE,
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS admin_password_reset (
  id BIGSERIAL PRIMARY KEY,
  admin_id BIGINT NOT NULL REFERENCES utenti_admin(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS ordini (
  ...
  cliente_id BIGINT NOT NULL REFERENCES clienti(id) ON DELETE RESTRICT,
...
CREATE TABLE IF NOT EXISTS righe_ordini (
  ...
  articolo_id BIGINT REFERENCES articoli(id) ON DELETE SET NULL,
  variante_id BIGINT REFERENCES varianti(id) ON DELETE SET NULL,
...
CREATE TABLE IF NOT EXISTS spedizioni (
  id BIGSERIAL PRIMARY KEY,
  ordine_id BIGINT NOT NULL UNIQUE REFERENCES ordini(id) ON DELETE CASCADE,
...
SELECT
  ...
  'EUR'
WHERE NOT EXISTS (
  SELECT 1 FROM impostazioni_negozio
);

INSERT INTO categorie (nome, slug, ordinamento)
VALUES
...
INSERT INTO colori (nome, codice_hex)
VALUES
  ('Nero', '#000000'),
  ('Bianco', '#FFFFFF'),
  ('Grigio', '#808080'),
...
INSERT INTO taglie (valore, ordinamento)
VALUES
  ('35', 350),
  ('36', 360),
  ('36.5', 365),
